...
for i=length(Megadatabase):-1:1
    ...
    Mega1D.Subject(i)=Megadatabase(i).Subject';
...
Mega1D.Subject=Mega1D.Subject(~isnan(Mega1D.Forward(1,:)))';
...
g(1,1)=gramm('x',repmat([1:100]',1,length(Mega1D.Sex))','y',Mega1D.Forward, 'color',Mega1D.Sex,'linestyle',Mega1D.Time,'subset', strcmp(Mega1D.Stat, Statistic) & ismember(Mega1D.Signal, Signals) & ~ismember(Mega1D.Subject,BadS));
...
g(1,2)=gramm('x',Mega0D.Time,'y',Mega0D.data, 'color',Mega0D.Sex,'subset', strcmp(Mega0D.Stat, Statistic) & ismember(Mega0D.Signal, Signals) & ~ismember(Mega0D.Subject,BadS));
